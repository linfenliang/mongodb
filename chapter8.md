...
片键是决定查询落到哪一个chunk上的依据，这个信息保存在配置服务器上，而索引是针对每个片上的集合来说的，每个片都会为自己的集合创建索引数据，因此，**查询语句中是否包含片键与索引对查询性能影响较大**。
可以通过explain解释执行查看，需要注意几个点：

nscanned:查询用到索引时，索引中扫描的文档数
nscannedObjects:在集合中扫描的文档数
nscannedAllPlans:所有查询计划中扫描的文档总数
nscanned:选定的某一种查询计划下扫描的文档总数
